...
if __name__ == "__main__":
    ...
    # Les poids "best.pt" servent de base d’initialisation (transfer learning)
    # Cela accélère la convergence et améliore les performances avec peu de données.
    model = YOLO('../yolo_models/best.pt')

    # Chemin vers le fichier data.yaml décrivant le dataset (train / validation / classes)
    datapath = Path('../dataset/data.yaml')
    ...
    # results = model.predict("../path/to/test2.mp4", show=True, save=True)